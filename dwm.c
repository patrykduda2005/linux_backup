...
swal(Client *swer, Client *swee, int manage)
{
	...
	*pc = swee;
	...
	resize(swee, swer->x, swer->y, swer->w, swer->h, 0, 0);
...
swalstop(Client *swee, Client *root)
{
	...
	if (swer->isfloating || !root->mon->lt[root->mon->sellt]->arrange) {
		...
		resize(swer, swee->x, swee->y, swee->w, swee->h, 0, 0);
	}
...
updatetitle(Client *c)
{
    ...
    strcpy(c->class, ch.res_class ? ch.res_class : broken);
